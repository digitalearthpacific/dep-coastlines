...
def remove_small_areas(bool_da: DataArray, min_size_in_pixels: int = 55) -> DataArray:
    def _remove_2d(bool_da_2d: DataArray) -> DataArray:
        ...
        return bool_da_2d.where(zones.isin(big_zones) == 1, False)
...
def contours_preprocess(
    ...
    land_mask = land_mask & other_land_mask

    nodata = combined_ds[masking_index].isnull()
    land_mask = land_mask.where(~nodata, False)
    ...
    if remove_inland_water:
        water = ~land_mask
        inland_water = find_inland_areas(water, esa_ocean)
        land_mask = land_mask & ~inland_water
    ...
    if mask_temporal:
        ...
        temporal_mask = temporal_masking(land_mask)

        land_mask = land_mask.where(temporal_mask, False)

    if mask_esa_water_land:
        close_to_coast = odc.algo.mask_cleanup(esa_ocean, [("dilation", 10)])
        land_mask = land_mask.where(close_to_coast, False)

    if remove_tiny_areas:
        # This was created mainly for surf artifacts in nir08, and may not be
        # needed if using e.g. mndwi
        land_mask = land_mask.where(remove_small_areas(land_mask), False)

    # Expand the land mask a bit to ensure we include the actual coast
    # land_mask = odc.algo.mask_cleanup(land_mask, [("dilation", 3)])
    breakpoint()
