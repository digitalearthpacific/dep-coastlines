...
aoi = gpd.read_file("data/aoi.gpkg").to_crs(8859)
...
aoi_negative_buffer = aoi.buffer(-buffer_distance_m)

coast_buffer = aoi_buffer.difference(aoi_negative_buffer).unary_union
...
pathrows.geometry = make_valid(pathrows.geometry)

intersection = pathrows.intersection(make_valid(coast_buffer))
