...
class Cleaner(Processor):
    ...
    def _calculate_mask(self, input):
        ...
        for year in input.year:
            ...
            ).Predictions.isin(masked_values)
    ...
    def process(self, input: Dataset | list[Dataset]) -> Tuple[Dataset, GeoDataFrame]:
        ...
        if isinstance(input, list):
            first_mask = self._calculate_mask(input[0])
            output = input[0][self.water_index].where(~first_mask)
            for ds in input[1:]:
                ds = ds.sel(year=ds.year[ds.year.isin(output.year)])
                this_mask = self._calculate_mask(ds)
                output = output.where(
                    ~output.isnull(), ds[self.water_index].where(~this_mask)
                )
        else:
...
def run(task_id: Tuple | list[Tuple] | None, dataset_id=DATASET_ID) -> None:
    ...
    loader = MultiyearMosaicLoader(start_year, end_year, years_per_composite=[1, 3, 5])
    processor = Cleaner()
